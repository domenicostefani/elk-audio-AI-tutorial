...
public:
    /** Constructor */
    InterpreterWrap(const std::string &filename, bool verbose = false);
...
InterpreterWrap::InterpreterWrap(const std::string &filename, bool verbose) {
    ...
    if (verbose)
        std::cout << "Done.\nPriming the Interpreter (Calling inference once)..." << std::endl;
...
int InterpreterWrap::classify_internal(const float featureVector[], size_t numFeatures, float outputVector[], size_t numClasses, bool verbose) {
    if (verbose) {
...
int classify(InterpreterPtr cls, const float featureVector[], size_t numFeatures, float outputVector[],
...
int classifyFlat2D(InterpreterPtr cls, const float flatFeatureMatrix[], size_t nRows, size_t nCols, float outputVector[], size_t numClasses, bool verbose) {
...
int classifyFlat2D(InterpreterPtr cls, std::vector<float> &flatInputMatrix, size_t nRows, size_t nCols, std::vector<float> &outputVector, bool verbose) {
